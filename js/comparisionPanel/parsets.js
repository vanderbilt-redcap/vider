...
define(["require","comparisionPanel"],
    ...
    d3.parsets = function() {
        ...
        function parsets(selection) {
            selection.each(function(data, i) {
                ...
                function updateRibbons() {
                    ...
                    mouse
                        .on("click", function(d){
                            require("comparisionPanel").update(d);
                        });
                    mouse.exit().remove();
                }
                ...
                function updateCategories(g) {
                    ...
                    category.select("rect")
                        .attr("class", function(d) {
                            return d.dimension === dimensions[0] ? "parasets category-" + ordinal(d.name) : "parasets";
                        })
        ...
        //var divHeight = document.getElementById("comparisionPanel").clientHeight;
        var divHeight = window.innerHeight - 200;
        //console.log('Div Height - ', divHeight);

        return d3.rebind(parsets, event, "on").value(1).width(divWidth).height(divHeight);

        function dimensionFormatName(d, i) {
            var displayText = d.name.substring(0,50) +  (d.name.length > 50 ? "..." : "");
            d.complete_name = d.name;
            return dimensionFormat.call(this, displayText, i);
        }
