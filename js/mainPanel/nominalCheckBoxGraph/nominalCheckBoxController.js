 ...
 */
define(["nominalCheckBoxView", "dataWrapper", "filterData"],
    function (nominalCheckBoxView, dataWrapper, filterData) {
        ...
        var createNominalStratObj = function(stratCategories , stratDataObj){
            ...
            stratCategories.forEach(function (strat) {
                ...
                var totalKeysLength = self.varData ? Object.keys(self.varData[0]).length : 0;

                for (var i = 0; i < self.varData.length; i++) {

                    //this key is good for nominal data
                    for(var key in self.varData[i]){

                        //if set then consider
                        if(1 == self.varData[i][key]){

                            //this to set the total count
                            origKeyValuePair[key].totalCount = filterCount * totalKeysLength;//self.varData.length;
                            ...
                            //this will contain the filtered value of the hover data
        ...
        var createNumericalStratObj = function(stratCategories, stratDataObj){
            ...
            stratCategories.forEach(function (strat) {
                ...
                //create label
                var stratValue = strat.x + " - " + (strat.x + strat.dx);
                ...
                //this will contain the selected filter query
                var filtering = require("filterData").getQuery();
                //this will contain the filtered data on hover
                var filterHover = filterData.getHoverArr();
                ...
                for (var i = 0; i < self.varData.length; i++) {
                    ...
                    //this will contain the filtered value of the SELECTED data
                    if ((null == filtering || true == filtering[i])
        ...
        var _create = function (_obj, _data, _container, _isQueried) {
            ...
            nominalCheckBoxView.create(_container,
                self.formName,
                self.varName,
